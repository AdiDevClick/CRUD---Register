...
export class Carousel {
    ...
    #createPagination() {
        ...
        this.#onMove((index) => {
            ...
            if (this.options.infinite) {
                ...
                // const calculatedButton = this.#calculate(
                //     itemsCount,
                ...
                activeButton =
                    ...
                    this.buttons[
                        // calculatedButton
                        // this.#calculate(itemsCount, previewsActiveButton)
                        Math.floor(
                            (index % (this.items.length - 2 * this.#offset)) /
                                this.#slidesToScroll
                        )
                    ];
            } else {
    ...
    #testcalculateActivePage(currentIndex, totalItems, itemsPerSlide, offset) {
        // Longueur totale du tableau
        const totalArrayItems = totalItems + 2 * offset;
        // Position réelle dans le tableau
        // let realIndex = Math.floor(
        //     ((currentIndex - offset) % totalItems) / itemsPerSlide
        // );
        // let realIndex = Math.floor((currentIndex % totalItems) / itemsPerSlide);
        let realIndex =
            (currentIndex - offset + totalArrayItems) % totalArrayItems;
        // // Ajustement pour la remise à zéro
        if (currentIndex >= totalArrayItems - offset) {
            console.log("JE SUIS AU DESSUS");

            // realIndex = realIndex % totalItems;
            // Rester sur la dernière page jusqu'à ce qu'on soit prêt pour une transition fluide
            // return (
            //     Math.floor((realIndex - totalItems) / itemsPerSlide) %
            //     totalItems
            // );

            // realIndex =
            //     (currentIndex - totalArrayItems + totalItems + offset) %
            //     totalItems;
            realIndex =
                (realIndex - totalItems + totalArrayItems) % totalArrayItems;
            // Réaligner les index pour éviter les sauts
            if (realIndex < 0) {
                realIndex += totalItems;
            }

            console.log("\n real Index au DESSUS => ", realIndex);
            // realIndex = (realIndex - totalItems + totalArrayItems) % totalItems;
            // Assurer que realIndex n'est pas négatif realIndex = realIndex < 0 ? realIndex + totalItems : realIndex;
        }

        // Calcul de la page active
        // return Math.floor(realIndex);
        return Math.floor(realIndex / itemsPerSlide);
    }

    #calculate(itemsCount, previewsActiveButton = null) {
        ...
        //     modulo / this.#slidesToScroll > this.#slidesToScroll - 1
        // ) {
    ...
    #calculateTotalPages(totalItems, itemsPerPage, itemsPerSlide) {
        return Math.ceil((totalItems - itemsPerPage) / itemsPerSlide) + 1;
    }
    ...
    #calculateActivePage() {
        const realIndex =
            (this.currentItem - this.#offset + this.items.length) %
            this.items.length;
        const adjustedIndex = realIndex - this.#offset;
        const totalPages =
            Math.ceil(
                (this.items.length - 2 * this.#offset - this.#visibleSlides) /
                    this.#slidesToScroll
            ) + 1;
        if (adjustedIndex < 0) return 0;
        return Math.floor(adjustedIndex / this.#slidesToScroll) % totalPages;
    }
    ...
    #resetInfinite() {
        if (this.currentItem <= this.#slidesToScroll) {
            console.log("J'ai demandé le reset + 10");
            this.goToItem(
