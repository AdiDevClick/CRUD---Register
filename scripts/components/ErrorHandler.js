...
import {
    ...
    emptyAlert,
    formButton,
    formIDToAvoidChecking,
    hiddenAlertClass,
    hiddenClass,
    inputErrorClass,
    inputsCanBeEmpty,
    inputsCanContainSpecialChars,
    inputsNotToAppend,
    inputsToListen,
    inputValidClass,
    invalidEmailMessage,
    invalidPwMessage,
    noSpaceAllowedMessage,
    notANumberError,
    notIdenticalPasswords,
    pwCannotBeUsername,
    pwIsTooCommon,
    sectionToWatch,
    strongPwDigitInputRegex,
    strongPwLengthInputRegex,
    strongPwLowerCaseInputRegex,
    strongPwSpecialCharInputRegex,
    strongPwUpperCaseInputRegex,
    thisInputShouldBeInt,
    tooltip,
    userInputRegex,
    wrongNumber,
} from "../configs/ErrorHandler.config.js";
import {
    alertMessage,
    createElement,
    debounce,
    filterArrayToRetrieveUniqueValues,
    retrieveUniqueNotAllowedCharFromRegex,
    setObjectPropertyTo,
...
export class ErrorHandler {
     ...
     */
    #thisInputIDShouldBeInt = Array.from(
        document.querySelectorAll(thisInputShouldBeInt)
    );
    /** @type {String} */
    #password;
    /** @type {String} */
    ...
    constructor(form, options = {}) {
        ...
        this.options = Object.assign(
            {},
            {
                ...
                whichInputAllowSpecialCharacters:
                    this.#inputsCanContainSpecialChars,
                createTooltips: true,
        ...
        if (!this.#alert) {
            ...
            if (this.#form.id !== this.#formIDToAvoidChecking)
                this.#form.insertAdjacentElement("afterbegin", this.#alert);
        }

        if (!this.options.debouncing) this.options.debounceDelay = 50;
        this.#listenInputs = Array.from(
            this.#form.querySelectorAll(this.#inputsToListen)
        );
        ...
        this.#listenInputs.forEach((input) => {
            ...
            // Automatically resize textarea depending on the inner content
    ...
    #dynamicCheck(input) {
        ...
        this.triggerToolTip();
        // Modify textarea size while typing
        this.#resizeTextarea(input);
        ...
        if (input.isEmpty) {
            ...
            this.#displayErrorMessage(
                `Les caractères suivants ne sont pas autorisés : ${this.#wrongInput} `,
                input
            );
            return false;
        } else if (
            input.id === "email" &&
            !this.#emailInputRegex.test(input.value)
        ) {
            ...
            this.#count = filterArrayToRetrieveUniqueValues(
                this.#count,
                input,
                "input"
            );
    ...
    #createIconContainer(input) {
        this.#inputsNotToAppend = Array.from(
            document.querySelectorAll(
                this.options.inputsNotToAppendIcon
                    ? this.options.inputsNotToAppendIcon
                    : inputsNotToAppend
            )
        );
    ...
    #createTooltipContainer(element) {
        const noTooltipInput = this.activeInput(
            this.#inputsNotToAppend,
            element
        );
    ...
    #displayErrorMessage(message, element) {
        ...
        if (element === this.#password)
            this.#pwdRepeat.classList.add(this.#inputErrorClass);
        if (element === this.#pwdRepeat)
    ...
    #charsNotAllowed(input) {
        ...
        if (
            (!input.allowSpecialCharacters ||
                input.allowSpecialCharacters == undefined) &&
            !this.#allowedSpecialChars.test(input.value)
        ) {
            // Retrieve every character that isn't allowed and only unique entries
            console.log(input);
            this.#wrongInput = retrieveUniqueNotAllowedCharFromRegex(
                input.value,
    ...
    /**
     * Modifie la hauteur du textarea en fonction
     * du texte inséré
    ...
    /**
     * Permet de modifier la hauteur d'un textarea
     * @returns
     */
    get resizeTextarea() {
        return (element) => {
            return this.#resizeTextarea(element);
        };
    }
     ...
     */
    #isANumber(input) {
        let inputShouldBeInt = this.activeInput(
            this.#thisInputIDShouldBeInt,
            input
        );
    ...
    #isEmptyInputs(input) {
        ...
        if (
            !input.canBeEmpty &&
            (input.value.toString().trim() === "" || !input.value)
        ) {
            // this.#isEmpty = true
            input.isEmpty = true;
            // return false
    ...
    #isExactPassword() {
        if (this.#password && this.#pwdRepeat) {
            if (
                this.#password.value !== this.#pwdRepeat.value ||
                this.#pwdRepeat.value !== this.#password.value ||
                this.#password.isEmpty ||
                this.#pwdRepeat.isEmpty ||
                !this.#password.value ||
    ...
    #isInputChecked(event) {
        ...
        for (const [key, value] of data) {
            ...
            if (
                key.includes("Mot de Passe" || "Mot de Passe de confirmation")
            ) {
                setObjectPropertyTo(
                    this.options.strongPassword,
                    arrayKey[key],
        ...
        for (const key in arrayKey) {
            ...
            if (isArray) {
                for (const el of target) {
                    element = Array.from(
                        el.querySelector(arrayKey[key].id)
                    ).filter((t) => t);
                }
                // target.forEach(el => {
    ...
    #checkBatchOfInputs(array) {
        ...
        for (const key in arrayKey) {
            ...
            for (const element of array) {
                if (!element) continue;
                let elementToCheck = this.#findElement(
                    element,
                    arrayKey[key].id.split("#")[1]
                );
